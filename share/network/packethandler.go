...
func (pk *PacketHandler) Handle(args *PacketArgs) {
	...
	if pk.packets[uint16(args.Type)] == nil {
		// unknown packet received
		log.Errorf("Unknown packet received (Len: %d, type: %d, src: %s)",
			args.Reader.Size, args.Type, args.Session.GetEndPnt(),
		)
	...
	var invoke = pk.packets[uint16(args.Type)].Method
	if invoke == nil {
	...
	log.Debugf("Handle: Session UserIdx: %d, Args UserIdx: %d", args.Session.UserIdx, args.UserIdx)
