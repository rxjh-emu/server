...
func OnPacketReceive(e *event.Event) {
	...
	log.Debugf("Received Packet `%s` (Len: %d, type: %d, UserIdx: %d, src: %s)",
		g_PacketHandler.Name(a.Type), a.Length, a.Type, a.UserIdx, a.Session.GetEndPnt(),
	)
...
func OnPacketSend(e *event.Event) {
	...
	log.Debugf("Sent Packet `%s` (Len: %d, type: %d, UserIdx: %d, src: %s)",
		g_PacketHandler.Name(a.Type), a.Length, a.Type, a.UserIdx, a.Session.GetEndPnt(),
	)
}
