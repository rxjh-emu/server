...
func (sm *ServerManager) GetGameServerList(serverName string) []server.ServerItem {
	...
	for _, value := range sm.servers {
		...
		var chn = server.ChannelItem{
			Id:           value.ChannelId,
			Name:         fmt.Sprintf("%s - %d", serverName, value.ChannelId),
			Type:         value.Type,
			Ip:           value.PublicIp,
		...
		var svr = &server.ServerItem{Id: value.ServerId, Name: serverName}
		var found = false
